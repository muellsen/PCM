...
% Options for prox
defopts.abstol = 1e-3;      % Primal-dual gap tolerance
defopts.gapit = 1e0;        % Interval for checking primal dual gap
defopts.maxit = p*1e2;        % Number of iteration in Coordinate descent
defopts.sigma_0_fac = 1e-2; % Data-dependent scaling of lower bound on sigma
defopts.lamlength = 200;    % Length of lambda path
...
for i = 1:nLams
    ...
    for k=1:maxit
        ...
        for j=actInds
            ...
            sigma_i = max(sigma_0,sqrt(sum((Yvec-Xmat*beta_i).^2))/sqrt_n);
            %sigma_i = max((XY-XX*beta_i))/n; % TREX style
        end 
    end
