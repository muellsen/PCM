function [betaSQMat, sigmaSQMat,funSQMat,out] = sqrt_lasso_ul(Xmat, Yvec, inopts)
% Sqrt Lasso with Douglas Rachford in unlocbox style
...
defopts.lenPath = 20;       % 200 Length of lambda path
...
% Use unlocbox for Douglas-Rachford
init_unlocbox();
...
for i = 1:nLams
    ...
    % Setting the proximity operators
    
    clear param_l2;
    
    % Data fitting (L2 norm)
    param_l2.A = A;
    param_l2.At = At;
    param_l2.y = Yvec;
    param_l2.verbose = 0;
    param_l2.tight = 0;
    param_l2.nu = 1;
    f3.prox=@(x,T) prox_l2(x,lam_i*T,param_l2);
    f3.grad=@(x) 2*lam_i*A(A(x)-Yvec);
    f3.eval=@(x) lam_i*norm(A(x)-Yvec,'fro');
    
    % Regularization parameter (L1 norm)
    param_l1.verbose = 0;
    ...
    param.maxit = maxit;
    param.tol = 1e-3;
    param.lambda = opts.dr_mu;
    
    solSqrt = douglas_rachford(beta_0,f1,f3,param);
