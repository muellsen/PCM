function betaT = proxL1s(b,lam,gam,weights)
% Proximity operator of L1 norm with zero-sum constraints (modified from tfocs)
...
temp1 = prox_l1sum( b(cInds), lam_gam(cInds(1)), 1e-12 );
...
function x = prox_l1sum( x0, lambda, b )
    ...
    if lwrBnd == 0 && (uprBnd ~= length(brk_pts) + 1)
        a2 = brk_pts( uprBnd );
        a1 = a2 - 10; % arbitrary
        aBounds = [-Inf,a2];
    elseif uprBnd == length(brk_pts) + 1 && (lwrBnd ~= 0)
        a1 = brk_pts( lwrBnd );
        a2 = a1 + 10; % arbitrary
        aBounds = [a1,Inf];
    else
